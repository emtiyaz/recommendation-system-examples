function [post nlZ dnlZ] = infEP(hyp, mean, cov, lik, x, y, options)
...
[recordPath, recordPathIters] = myProcessOptions(options,'recordPath',0,'recordPathIters', 0);
...
if recordPath % store initial point
    sW = sqrt(ttau);
    L = chol(eye(n) + (sW*sW').*Sigma);
...
% store - added by Emt
%x = [mu(:); packcovariance(Sigma)];
tt = toc;
fval = nlZ;
fprintf('%d %f\n',length(fval),fval(end));
...
while (abs(nlZ-nlZ_old) > tol && sweep < max_sweep) || sweep<min_sweep
  ...
  tic
  for i = 1:n%randperm(n)       % iterate EP updates (in random order) over examples
  ...
  % store - added by Emt
  tt(:,end+1) = toc;
  %x(:,end+1) = [mu(:); packcovariance(Sigma)];
  fval(:,end+1) = nlZ;
  fprintf('%d %f\n',length(fval),fval(end));
  ...
  if recordPath
      sW = sqrt(ttau);
      alpha = tnu-sW.*solve_chol(L,sW.*(K*tnu));
      trace.alpha(:,end+1) = alpha(:);
      trace.slambda(:,end+1) = sW(:);
  end
end
...
% Count flops - to check
singleSweep = countFlops([], 'ep', struct('D',n));
post.nflops = [0, cumsum(ones(1,sweep).*singleSweep)];
